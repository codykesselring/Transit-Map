...
(function buildEnv() {
  // EPA emission factors (lbs CO2 per passenger mile)
  // Sources: EPA 2023, APTA, MTA sustainability report
  const CAR_LBS_PER_MILE    = 0.89;  // avg passenger car, solo driver
  const BUS_LBS_PER_MILE    = 0.14;  // MTA bus (diesel, ~25 avg passengers)
  const SUBWAY_LBS_PER_MILE = 0.05;
  const CARPOOL_LBS_PER_MILE = 0.45; // 2-person carpool
  const WALK_LBS_PER_MILE   = 0.00;
  const AVG_BUS_PASSENGERS  = 25;

  // ── Infographic: 1 bus = X cars ──────────────────────────────────────────
  const infEl = document.getElementById('busCarInfographic');
  const carsReplaced = AVG_BUS_PASSENGERS;
  let html = `<div class="infographic-block">
    <div class="infographic-icon">🚌</div>
    <div class="infographic-count">1 Bus</div>
    <div class="infographic-label">full bus (${AVG_BUS_PASSENGERS} riders)</div>
  </div>
  <div class="infographic-eq">=</div>
  <div class="infographic-block">
    <div style="display:flex;flex-wrap:wrap;gap:4px;justify-content:center;max-width:320px">`;
  for (let i = 0; i < carsReplaced; i++) {
    html += `<span style="font-size:1.4rem" title="1 car trip removed">🚗</span>`;
  }
  html += `</div>
    <div class="infographic-count">${carsReplaced} cars</div>
    <div class="infographic-label">removed from the road</div>
  </div>
  <div class="infographic-eq" style="margin-left:1rem">=</div>
  <div class="infographic-block" style="margin-left:0.5rem">
    <div class="infographic-icon">🌳</div>
    <div class="infographic-count">${(AVG_BUS_PASSENGERS * CAR_LBS_PER_MILE * 3 / 48).toFixed(1)} trees</div>
    <div class="infographic-label">worth of CO2 per 3-mile trip</div>
  </div>`;
  infEl.innerHTML = html;

  // ── Emissions bar chart ───────────────────────────────────────────────────
  const modes  = ['Solo Car', '2-Person Carpool', 'MTA Bus', 'Subway/Rail', 'Walking/Biking'];
  const lbsPm  = [CAR_LBS_PER_MILE, CARPOOL_LBS_PER_MILE, BUS_LBS_PER_MILE, SUBWAY_LBS_PER_MILE, WALK_LBS_PER_MILE];
  const emColors = ['#f87171','#fbbf24','#38bdf8','#34d399','#a78bfa'];

  new Chart(document.getElementById('emissionsBar'), {
    type: 'bar',
    data: {
      labels: modes,
      datasets: [{
        label: 'lbs CO2 / passenger mile',
        data: lbsPm,
        backgroundColor: emColors.map(c => c + '33'),
        borderColor: emColors,
        borderWidth: 1.5,
        borderRadius: 4,
      }]
    },
    options: {
      ...
      plugins: {
        legend: { display: false },
        tooltip: { callbacks: { label: ctx => ` ${ctx.parsed.y} lbs CO2/mi` } }
      },
      scales: {
        x: { grid: { display: false } },
        y: { grid: { color: '#1e293b' }, beginAtZero: true, ticks: { callback: v => v + ' lbs' } }
      }
    }
  });

  // ── Annual CO2 savings line chart (2019–2024) ─────────────────────────────
  // savings = ridership × avg trip miles × (car_lbs - bus_lbs) / 2000 (tons)
  const AVG_TRIP_MI = 3.2;
  const ridershipM  = [29.1,28.7,30.2,29.8,30.5,29.9,28.8,30.1,31.2,30.8,29.4,27.6,
                        30.1,29.5, 8.2, 2.1, 1.5, 2.4, 4.8, 7.2,10.1,12.5,14.2,15.8,
                        16.9,17.4,18.0,17.1,18.5,19.2,18.8,19.6,20.5,14.3,15.6,16.2,
                        17.5,18.2,19.4,20.1,21.3,22.0,21.8,22.5,23.1,22.8,21.4,20.9,
                        21.5,22.3,23.8,24.5,25.2,25.8,24.9,25.7,26.3,25.9,24.6,23.8,
                        24.5,25.1,26.8,27.3,27.9,27.5,26.8,27.4,28.1,27.8,26.5,25.9];
  const years = ['2019','2020','2021','2022','2023','2024'];
  const annualSavings = years.map((_, yi) => {
    const monthlyRiders = ridershipM.slice(yi * 12, yi * 12 + 12);
    const totalRiders = monthlyRiders.reduce((a, b) => a + b, 0) * 1e6;
    return Math.round(totalRiders * AVG_TRIP_MI * (CAR_LBS_PER_MILE - BUS_LBS_PER_MILE) / 2000);
  });
  ...
  new Chart(document.getElementById('co2Line'), {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        tooltip: { callbacks: { label: ctx => ` ${ctx.parsed.y.toLocaleString()} tons CO2 saved` } }
      },
      scales: {
        x: { grid: { display: false } },
        y: { grid: { color: '#1e293b' }, beginAtZero: true, ticks: { callback: v => (v/1000).toFixed(0) + 'K t' } }
      }
    }
  });
  ...
  function calcResults() {
    ...
    const weeks = parseFloat(document.getElementById('calc-weeks').value) || 50;

    const annualMiles   = miles * 2 * days * weeks;        // round-trip
    const carLbsCO2     = annualMiles * CAR_LBS_PER_MILE;
    const busLbsCO2     = annualMiles * BUS_LBS_PER_MILE;
    const savedLbs      = carLbsCO2 - busLbsCO2;
    const savedTons     = savedLbs / 2000;
    const gasGallons    = annualMiles / mpg;
    const gasCost       = gasGallons * 3.50;               // ~$3.50/gal avg
    const treesEquiv    = savedLbs / 48;                   // 1 tree absorbs ~48 lbs CO2/yr

    document.getElementById('calc-results').innerHTML = `
      <div class="calc-result-card">
        <div class="calc-result-value">${savedLbs.toLocaleString(undefined,{maximumFractionDigits:0})} lbs</div>
        <div class="calc-result-label">CO2 saved per year</div>
      </div>
      <div class="calc-result-card">
        <div class="calc-result-value">${savedTons.toFixed(1)} tons</div>
        <div class="calc-result-label">CO2 (metric)</div>
      </div>
      <div class="calc-result-card">
        <div class="calc-result-value">${treesEquiv.toFixed(0)}</div>
        <div class="calc-result-label">trees equivalent</div>
      </div>
      <div class="calc-result-card">
        <div class="calc-result-value">${gasGallons.toFixed(0)} gal</div>
        <div class="calc-result-label">gas not burned</div>
      </div>
      <div class="calc-result-card">
        <div class="calc-result-value">$${gasCost.toLocaleString(undefined,{maximumFractionDigits:0})}</div>
        <div class="calc-result-label">estimated fuel saved</div>
      </div>
    `;
  }

  ['calc-miles','calc-days','calc-mpg','calc-weeks'].forEach(id =>
    document.getElementById(id).addEventListener('input', calcResults)
  );
  calcResults(); // run on load
})();
